...
echo -e "${BLUE}Excluding: node_modules, .next, .git directories, and binary files${NC}"
echo "--------------------------------------------------------"
...
total_files=$(find . -type f -not -path "*/node_modules/*" -not -path "*/.next/*" -not -path "*/.git/*" | grep -v -E "\.(${BINARY_EXTENSIONS})$" | wc -l)
echo -e "${GREEN}Total text files:${NC} $total_files"

# Count binary files
binary_files=$(find . -type f -not -path "*/node_modules/*" -not -path "*/.next/*" -not -path "*/.git/*" | grep -E "\.(${BINARY_EXTENSIONS})$" | wc -l)
...
total_stats=$(find . -type f -not -path "*/node_modules/*" -not -path "*/.next/*" -not -path "*/.git/*" | grep -v -E "\.(${BINARY_EXTENSIONS})$" | xargs wc -l -c 2>/dev/null | tail -n 1)
...
find . -type f -not -path "*/node_modules/*" -not -path "*/.next/*" -not -path "*/.git/*" | grep -v "count-lines.sh" | grep -v -E "\.(${BINARY_EXTENSIONS})$" | sed 's/.*\.//' | sort | uniq -c | sort -nr | while read count ext; do
    if [ "$ext" != "" ]; then
        # Get line and character count for this extension
        ext_stats=$(find . -type f -name "*.$ext" -not -path "*/node_modules/*" -not -path "*/.next/*" -not -path "*/.git/*" | xargs wc -l -c 2>/dev/null | tail -n 1)
...
find . -type f -not -path "*/node_modules/*" -not -path "*/.next/*" -not -path "*/.git/*" | grep -E "\.(${BINARY_EXTENSIONS})$" | sed 's/.*\.//' | sort | uniq -c | sort -nr | while read count ext; do
    if [ "$ext" != "" ]; then
